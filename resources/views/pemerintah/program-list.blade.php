    ...
    <div class="container mx-auto px-4 md:px-6">
                    ...
                    <div class="p-4">
                        ...
                        <div class="flex items-center mb-3">
                            <div class="w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center text-primary font-bold text-sm">
                                {{ substr($program->opd_name, 0, 1) }}
                            </div>
                            <div class="ml-2">
                                <p class="text-xs font-medium text-dark truncate max-w-[120px]">{{ $program->opd_name }}</p>
                                <p class="text-xs text-gray-500">Pemkab</p>
                            </div>
                        </div>
                        
                        <!-- Title & Description -->
                        <h3 class="text-sm font-semibold text-dark mb-2 line-clamp-2 leading-tight">{{ $program->title }}</h3>
                        <p class="text-gray-600 text-xs mb-3 line-clamp-2 leading-relaxed">{{ Str::limit($program->description, 80) }}</p>
                        ...
                        <div class="flex justify-between items-center pt-2 border-t border-gray-100">
                            <span class="text-xs text-gray-500 truncate max-w-[80px]">{{ $program->category }}</span>
                            <div class="flex gap-2">
                                <a href="{{ route('pemerintah.program.edit', $program->id) }}" 
                                   class="text-blue-600 hover:text-blue-800 text-xs font-medium">
                                    ✏️
                                </a>
                                <form action="{{ route('pemerintah.program.destroy', $program->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" 
        ...
        @else
            <div class="text-center py-12">
                ...
                <a href="{{ route('pemerintah.program.create') }}" class="inline-block bg-primary text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 transition text-sm">
